...
let logicBlockGeTest1 = 
    testCase "Logic Block Gen test 1" <| fun () -> 
        ...
        Expect.equal (match parserTest1Ans with | Ok ast -> convertAST ast) expected "Logic Block Gen test 1"   
 
[<Tests>]
let logicBlockGeTest2 = 
    testCase "Logic Block Gen test 2" <| fun () -> 
        let expected = logicBlockGenTest2Ans
        Expect.equal (match parserTest2Ans with | Ok ast -> convertAST ast) expected "Logic Block Gen test 2" 

[<Tests>]
let logicBlockGeTest3 = 
    testCase "Logic Block Gen test 3" <| fun () -> 
        let expected = logicBlockGenTest3Ans
        Expect.equal (match parserTest3Ans with | Ok ast -> convertAST ast) expected "Logic Block Gen test 3" 
