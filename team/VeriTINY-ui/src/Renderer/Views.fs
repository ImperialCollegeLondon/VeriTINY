...
let updateTable (lst) (tableID:HTMLElement) =  
    ...
    let makeContig (lst : (string * string) list) =
        ...
        let tr = createDOM "tr" <| List.map makeNode (["Net Name"; "Value (Bin)" ])
...
let clearSimulationTable() = 
    memList.innerHTML <- ""
    simulationCycleDisplay.innerHTML <- ""
    let display = ["Ready to Simulate",""]
    updateTable display simulationCycleDisplay
